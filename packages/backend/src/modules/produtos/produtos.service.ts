...
export class ProdutosService {
    ...
    async update(id: string, updateProdutoDto: UpdateProdutoDto) {
        ...
        if (updateProdutoDto.precoCompra || updateProdutoDto.precoVenda) {
            ...
            const precoCompra = updateProdutoDto.precoCompra || produtoAtual!.precoCompra;
            const precoVenda = updateProdutoDto.precoVenda || produtoAtual!.precoVenda;

            margemLucro =
                ((Number(precoVenda) - Number(precoCompra)) / Number(precoCompra)) * 100;
        }
